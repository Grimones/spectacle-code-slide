...
const {Slide, Layout, Fill} = require('spectacle');
...
class SplitViewCodeSlide extends React.Component {
  static propTypes = {
  ...
  render() {
    ...
    return (
      <Slide ref='slide' bgColor={slideBg} margin={1} {...rest}>
        ...
        <Layout>
          <Fill>
            <pre ref="container" style={{...style, height: window.innerHeight}}>
              <code style={{ display: "inline-block", textAlign: "left" }}>{lines}</code>
            </pre>
          </Fill>
          <Fill>
            <pre style={{...style, height: window.innerHeight}}>
